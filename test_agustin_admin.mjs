...
async function run() {
  ...
  if (uid) {
    ...
    // Check if his ID is stored differently maybe?
    console.log('Checking all client_trainer_assignments...');
    const allT = await db.collection('client_trainer_assignments').get();
    allT.forEach(d => {
        if (d.data().trainerId === uid || d.id === uid) {
            console.log('Found matching Assignment somehow:', d.id, d.data());
        }
    });
  }
}
