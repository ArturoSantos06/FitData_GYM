...
function NotificationCenter({ userId }) {
   ...
   useEffect(() => {
       ...
       // Configura la búsqueda por usuario activo y ordena por frescura (nuevo arriba)
       const q = query(
           collection(db, `users/${userId}/notifications`),
           orderBy('createdAt', 'desc')
       );

       // Empieza a sintonizar cada cambio sobre esa query
   ...
   // Computa qué número de elementos poner en la bolita roja sin leer.
   const unreadCount = notifications.filter(n => !n.read).length;

   // Realiza la transacción directa de una notificación "no vista" a Firestore.
   const markAsRead = async (notifId) => {
       const ref = doc(db, `users/${userId}/notifications`, notifId);
       await updateDoc(ref, { read: true }).catch(console.error);
       setIsOpen(false); // Esconde el panel de forma pulida al acceder a la respectiva info
   };
