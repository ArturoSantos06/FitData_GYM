...
function SeleccionarEntrenador() {
  const [entrenadorSeleccionadoClientes, setEntrenadorSeleccionadoClientes] = useState(null);
  ...
  return (
    <div className="w-full max-w-6xl mx-auto p-6">
        ...
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-slate-700/50">
            {ENTRENADORES_SIMULADOS.map((entrenador) => (
              <tr key={entrenador.id} className="hover:bg-slate-700/20 transition-colors group">
                {/* Perfil */}
                <td className="px-6 py-4">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <p className="text-white font-medium">{entrenador.name}</p>
                ...
                <td className="px-6 py-4">
                  <button 
                    onClick={() => setEntrenadorSeleccionadoClientes(entrenador)}
      ...
      {entrenadorSeleccionadoClientes && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-md p-6 shadow-2xl">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-lg font-bold text-white">Clientes de {entrenadorSeleccionadoClientes.name}</h3>
              <button 
                onClick={() => setEntrenadorSeleccionadoClientes(null)}
                className="text-slate-400 hover:text-white"
              >✕</button>
            </div>
            
            <div className="space-y-3">
              {entrenadorSeleccionadoClientes.clients.length > 0 ? (
                entrenadorSeleccionadoClientes.clients.map((cliente, idx) => (
                  <div key={idx} className="flex items-center gap-3 p-3 bg-slate-800/50 rounded-lg border border-slate-700/50">
                    <div className="w-2 h-2 rounded-full bg-purple-500"></div>
                    <span className="text-slate-200 text-sm">{cliente}</span>
                  </div>
                ))
              ) : (
