...
import BarraNavegacionEntrenador from '../portal/BarraNavegacionEntrenador';
...
function RutinaEntrenador() {
  ...
  const cambiarPestana = (pestana) => {
    if (pestana === 'agenda') {
      navigate('/entrenador', { state: { initialTab: 'agenda' } });
      return;
    }

    if (pestana === 'gestion') {
      navigate('/entrenador', { state: { initialTab: 'gestion' } });
      return;
    }

    if (pestana === 'inicio' || pestana === 'mensajes' || pestana === 'perfil') {
      navigate('/entrenador', { state: { initialTab: pestana } });
    }
  };
    ...
    <div className="min-h-screen bg-linear-to-br from-slate-950 via-slate-900 to-slate-950 text-white p-4 md:p-8">
      <div className="max-w-6xl mx-auto space-y-6">
        <BarraNavegacionEntrenador
          pestañaActiva="gestion"
          onCambiarPestaña={cambiarPestana}
          onCerrarSesion={cerrarSesion}
        />

        <div className="h-20 md:h-24" />

        <section className="bg-gray-800/55 p-4 md:p-6 rounded-xl shadow-xl border-t-4 border-purple-500">
