...
import { Calendar, Clock, User, XCircle, RefreshCw, AlertCircle, CheckCircle2 } from 'lucide-react';
            ...
            {appointments.length === 0 ? (
                ...
                <div className="space-y-4">
                    {appointments.map((cita) => {
                        // ==========================================
                        // LÓGICA DE TIEMPO: ¿La cita ya pasó?
                        // ==========================================
                        const ahora = new Date();
                        // Asumimos que horaFin es como "09:00", lo juntamos con la fecha "2026-04-18T09:00:00"
                        const fechaCita = new Date(`${cita.fecha}T${cita.horaFin || '23:59'}:00`);
                        const yaPaso = fechaCita < ahora;

                        // Determinamos el estado visual real
                        let estadoVisual = cita.estado;
                        if (estadoVisual !== 'cancelada' && yaPaso) {
                            estadoVisual = 'completada';
                        }

                        // Colores dinámicos del contenedor
                        let borderClass = 'border-slate-700 hover:border-emerald-500/50 hover:shadow-lg hover:shadow-emerald-900/20';
                        if (estadoVisual === 'cancelada') borderClass = 'border-red-500/20 opacity-75';
                        if (estadoVisual === 'completada') borderClass = 'border-blue-500/20 opacity-75 bg-slate-800/50';

                        return (
                            <div key={cita.id} className={`bg-slate-900 rounded-xl p-6 border transition-all ${borderClass}`}>
                                <div className="flex flex-col md:flex-row justify-between md:items-center gap-4">
                                    <div className="space-y-3 flex-1">
                                        ...
                                        <div className={`grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm ${estadoVisual === 'activa' ? 'text-slate-300' : 'text-slate-500'}`}>
                                            <div className="flex items-center gap-2"><Calendar size={16} className={estadoVisual === 'activa' ? 'text-emerald-500' : 'text-slate-500'} /><span>{cita.fecha}</span></div>
                                            <div className="flex items-center gap-2"><Clock size={16} className={estadoVisual === 'activa' ? 'text-emerald-500' : 'text-slate-500'} /><span>{cita.horaInicio} - {cita.horaFin}</span></div>
                                            <div className="flex items-center gap-2 sm:col-span-2"><User size={16} className={estadoVisual === 'activa' ? 'text-emerald-500' : 'text-slate-500'} /><span>Nutriólogo asignado (Contactar vía mensaje)</span></div>
                                        </div>
                                    </div>
                                    
                                    {/* BOTONES: Solo se muestran si está Activa (Ni cancelada, ni en el pasado) */}
                                    {estadoVisual === 'activa' && (
