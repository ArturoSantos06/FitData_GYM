...
const isTrainerAvailable = (trainer = {}) => {
  const role = String(trainer.role || trainer.user_type || '').toLowerCase();
  const status = String(trainer.trainerStatus || trainer.contractStatus || '').toLowerCase();
  return Boolean(
    ...
    const fetchData = async () => {
      try {
        ...
        querySnapshot.docs.forEach((docSnap) => {
          ...
          if (!trainerKey || seenTrainerKeys.has(trainerKey) || !isTrainerAvailable(trainer)) {
            return;
          }
      ...
      {entrenadores.length > 0 ? (
        ...
        >
          {entrenadores.map((e) => (
            <div
              key={e.id}
              className={`relative w-full h-full bg-slate-900/60 border rounded-xl p-6 transition-all duration-300 hover:bg-slate-800/80 hover:border-blue-500/50 hover:shadow-lg hover:shadow-blue-500/10 ${
            ...
            >
              ...
              <div className="flex h-full w-full flex-col items-center text-center space-y-4">
